...
class LoginTask()
{
    ...
    fun requestAction(email: String?, password: String?) {
        load.dismiss()
       val boyprefs = context.getSharedPreferences(
        ...
        val POST_ORDER: String = context.getResources().getString(R.string.utama)
        try {
                ...
                Response.ErrorListener { error ->
                    ...
                    if (error is ServerError && response != null) {
                        try {
                            ...
                            val hasil = obj.getString("message")
                            if (hasil.equals("Email not yet verified", ignoreCase = true)) {
                                context.startActivity(Intent(context, Verifikasi::class.java))
                                (context as Activity).finish()
                            } else {
                                Toast.makeText(
                                    context,
